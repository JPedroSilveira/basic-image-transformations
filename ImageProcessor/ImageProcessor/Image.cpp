...
namespace type 
{
    ...
    void Image::rotateLeft()
    {
        ...
        for (int row = 0; row < src.rows; row++)
        {
            for (int col = 0; col < src.cols; col++)
            {
                ...
                dst.at<Vec3b>(src.cols - 1 - col, row)[2] = srcColor(2);
                dst.at<Vec3b>(src.cols - 1 - col, row)[1] = srcColor(1);
                dst.at<Vec3b>(src.cols - 1 - col, row)[0] = srcColor(0);
            }
        }
    ...
    void Image::applyThreeByThreeMatrixConvolution(float filter[3][3])
    {
        if (this->isEmpty()) return;
        ...
        for (int row = 1; row < src.rows - 1; row++)
        {
            ...
            {
                neighborhoodChannels[1][1] = src.at<Vec3b>(row, col); // CENTER
                neighborhoodChannels[0][1] = src.at<Vec3b>(row - 1, col); // TOP
                neighborhoodChannels[2][1] = src.at<Vec3b>(row + 1, col); // BOTTOM
                neighborhoodChannels[1][0] = src.at<Vec3b>(row, col - 1); // LEFT
                neighborhoodChannels[1][2] = src.at<Vec3b>(row, col + 1); // RIGHT
                neighborhoodChannels[0][0] = src.at<Vec3b>(row - 1, col - 1); // TOP LEFT
                neighborhoodChannels[0][2] = src.at<Vec3b>(row - 1, col + 1); // TOP RIGHT
                neighborhoodChannels[2][0] = src.at<Vec3b>(row + 1, col - 1); // BOTTOM LEFT
                neighborhoodChannels[2][2] = src.at<Vec3b>(row + 1, col + 1); // BOTTOM RIGHT

                for (int x = 0; x < 3; x++)
                {
                    for (int y = 0; y < 3; y++)
                    {
                        colorNeighborhood[2][x][y] = neighborhoodChannels[x][y](2);
                        colorNeighborhood[1][x][y] = neighborhoodChannels[x][y](1);
                        colorNeighborhood[0][x][y] = neighborhoodChannels[x][y](0);
                    }
                }
